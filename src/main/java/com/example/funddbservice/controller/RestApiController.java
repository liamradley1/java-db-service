...
public class RestApiController {
    ...
    public ResponseEntity<Iterable<Fund>> getAllFunds() {
        return new ResponseEntity<>(fundDbService.getAllFunds().join(), HttpStatus.OK);
    }

    @GetMapping("/get-fund/{id}")
    @SneakyThrows
    public ResponseEntity<Fund> getFundInfo(@PathVariable("id") Long fundId) {
        Fund fund = fundDbService.getFundInfo(fundId).join();
    ...
    public ResponseEntity<Void> addFundInfo(@RequestBody Fund fund) {
        return new ResponseEntity<>(fundDbService.addFundInfo(fund).join() != null? HttpStatus.CREATED : HttpStatus.BAD_REQUEST);
    }

    @PutMapping("/put-fund/{id}")
    @SneakyThrows
    public ResponseEntity<Fund> updateFundInfo(@PathVariable("id") Long fundId, @RequestBody Fund fundToSave) {
        Fund updatedFund = fundDbService.updateFundInfo(fundId, fundToSave).join();
